...
public interface TblCommentStatisticDAO extends JpaRepository<CommentStatisticEntity,Integer> {
    @Query(value = "select * from comment_statistics where id=?",nativeQuery = true)
    CommentStatisticEntity findById(int id);

    @Query(value = "select * from comment_statistics where order_id=?",nativeQuery = true)
    CommentStatisticEntity findByOrderId(int order_id);

    @Modifying
    @Query(value = "insert into comment_statictics(order_id,content, rate,desc_star,logistics_star,attitude_star) values (?,?,?,?,?,?)",nativeQuery = true)
    void addCommentStatistic(int order_id,String content,int rate,int des_star,int logistics_star,int attitude_star);

    @Modifying
    @Query(value = "update comment_statictics set order_id = ?, content=?, rate=?, desc_star=?, logistics_star=?, attitude_star=? where  id= ?",nativeQuery = true)
    ...
    @Modifying
    @Query(value = "delete from comment_statics where id = ?",nativeQuery = true)
