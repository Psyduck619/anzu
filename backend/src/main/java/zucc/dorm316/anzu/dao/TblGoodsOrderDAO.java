...
public interface TblGoodsOrderDAO extends JpaRepository<GoodsOrderEntity,Integer> {
    ...
    @Modifying
    @Query(value = "insert into goods_order(user_id,goods_id, merchant_id, user_address_id, merchant_address_id, order_status, goods_num, goods_total_price, receiving_time, lease_time, double deposit) values(?,?,?,?,?,?,?,?,?,?,?)",nativeQuery = true)
    ...
    @Modifying
    @Query(value = "update goods_order set user_id = ?, goods_id=?, merchant_id = ?, merchant_id = ?, user_address_id = ?, merchant_address_id = ?, order_status = ?, goods_num = ?, goods_total_price = ?, receiving_time = ?, lease_time = ?,deposit_time=? where id = ?",nativeQuery = true)
